...
function select_one_it(str,cn){
 ...
 var obj2 = null ;
 var objcn  = null;
  obj2 = document.getElementById("chin_standard_typenum_1");
  objcn = document.getElementById("chin_typenum_1");
  //v2 = v2+ " " + str+ " ";
  v2 = v2+ "" + str+ "";
  //obj2.value = v2;
  objcn.value = v2;
...
function select_it(form){
	...
	obj1 = document.getElementById("chin_standard_typenum_1");
    objcn = document.getElementById("chin_typenum_1");
...
%>
	  <li>
		...
		<label for="int01">
		<a href="${pageContext.request.contextPath }/search/advanced.htm?cn_typenum=<%=cn%>" target="_parent"">
		<%=name%>
		</a>
		<%if(level!=4){
		 %>
		 <a id="orgfl_<%=i%>" href="#interClass"  flag="2" mylink="select_org_cn_fl.jsp?level=<%= level%>&ccscode=<%=cn%>&ccsptr=<%=java.net.URLEncoder.encode(name, "UTF-8")%>">
...
<%
	...
	if(i == 0){
		out.println("<li><h4>没有本级分类</h4></li>");
	}
}
%>
        </ul>
